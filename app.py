...
def ai_attendance_analysis():
    ...
    for a in attendance_data:
        sid = str(a.get('student_id'))
        if sid not in analysis_data:
            analysis_data[sid] = {'present': 0, 'absent': 0, 'name': student_map.get(sid, 'Unknown')}
        if a.get('status') == '1' or a.get('status') == 'Present':
            analysis_data[sid]['present'] += 1
        else:
            analysis_data[sid]['absent'] += 1

    raw_data_str = str(analysis_data)
    prompt = "Analyze attendance data and provide insights. The data is a dictionary mapping student IDs to their present/absent counts. Identify irregular students, calculate percentages, and suggest actions:\n" + raw_data_str

    c = get_openai_client()
    if not c: return jsonify({'success': False, 'error': 'OpenAI client not initialized'})
...
def ai_quiz_generator():
    ...
    prompt = f"Generate 5 MCQ questions with answers about {subject} - {topic}. Format as a list of questions, each with 4 options and the correct answer."
...
def ai_video_summary():
    ...
    prompt = f"Generate summary notes for revision for a class video about: {topic}. Include summary, key points, and revision notes."
...
def ai_study_chatbot():
    ...
    messages = [{"role": "system", "content": "You are a friendly tutor. The student can ask concept doubts, exam preparation, and study tips."}]
    for msg in history:
