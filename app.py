...
def ai_chat():
    ...
    prompt_type = data.get('type', 'doubt') # 'doubt' or 'chatbot'

    if prompt_type == 'doubt':
        ...
        system_prompt = "You are a friendly and helpful AI study tutor. You help students with concept doubts, exam preparation, and study tips in a very encouraging manner."

    messages = [{"role": "system", "content": system_prompt}]

    # history should be a list of dicts with 'role' and 'content'
    for msg in history:
    ...
    try:
        ...
        return jsonify({'success': False, 'error': 'Failed to get response from AI.'})
...
def ai_quiz():
    ...
    prompt = f"Generate 5 MCQ questions with answers for the subject '{subject}' on the topic '{topic}'. Return the output strictly as a JSON array of objects. Each object must have the following keys exactly: 'question', 'option1', 'option2', 'option3', 'option4', and 'answer'. The 'answer' must exactly match the text of the correct option."
    ...
    try:
        ...
        # Handle cases where GPT wraps the array in a key
...
def ai_announcement():
    ...
    try:
        ...
        reply = response.choices[0].message.content
        return jsonify({'success': True, 'announcement': reply})
    except Exception as e:
        print(f"OpenAI error: {e}")
        return jsonify({'success': False, 'error': 'Failed to generate announcement.'})
...
def ai_attendance():
    ...
    for a in attendance_data:
        ...
        if sid not in raw_data:
            raw_data[sid] = {'present': 0, 'absent': 0, 'name': student_map.get(sid, 'Unknown')}
    ...
    import json
    prompt = f"Analyze attendance data and provide insights. The raw data is: {json.dumps(raw_data)}. Summarize which students are irregular, their attendance percentage, and suggest actions."
    ...
    try:
        ...
        print(f"OpenAI error: {e}")
        return jsonify({'success': False, 'error': 'Failed to analyze attendance.'})
...
def ai_video_summary():
    ...
    try:
        ...
        print(f"OpenAI error: {e}")
        return jsonify({'success': False, 'error': 'Failed to generate video summary.'})
